    ...
    tdLogin = {
        ...
        handlerRegister : function() {
            ...
            var registerEmailEl = jQuery( '#register_email' ),
				...
				registerFacebookEl = jQuery( '#register_facebook' );

			
            if ( registerEmailEl.length && registerUserEl.length && registerFacebookEl.length  ) {
                var registerEmailVal = registerEmailEl.val().trim(),
                    registerUserVal = registerUserEl.val().trim(),
					registerFacebookVal = registerFacebookEl.val().trim();
					let checkFacebook = this.checkFacebookUrl(registerFacebookVal);	
                if ( tdLogin.email_pattern.test( registerEmailVal ) && registerUserVal && registerFacebookVal  && checkFacebook ) {

                    tdLogin.addRemoveClass( ['.td_display_err', 1, 'td_display_msg_ok'] );
                    tdLogin.showHideMsg( td_please_wait );

                    //call ajax
                    tdLogin.doAction( 'td_mod_register', registerEmailVal, registerUserVal, registerFacebookVal );
                } else {
...
(function(){
    ...
    tdLoginMob = {
        ...
        doAction : function( sent_action, sent_email, sent_user, sent_pass ) {
			...
			if('td_mod_register' == sent_action ){
				dataSend = {
					action: sent_action,
                    email: sent_email,
                    user: sent_user,
